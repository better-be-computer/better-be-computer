...
def load_split_dict(number):
    ...
    audio_location = "saved_objects/ASVspoof/ASVspoofTrain"
    process_files = []
...
class Dataset_train(Dataset):
    def __init__(self, list_IDs, labels, base_dir, paths):
    ...
    def __getitem__(self, index):
        ...
        X,_ = librosa.load(os.path.join(self.base_dir,path), sr=16000) 
...
if __name__ == '__main__':
    ...
    parser.add_argument('--iteration_numbers', type=str, default="0-4")

    # dir_yaml = os.path.splitext('model_config_RawNet')[0] + '.yaml'
    dir_yaml = "saved_objects/RawNet2/model_config_RawNet.yaml"
    ...
    assert track in ['DF'], 'Invalid track given'
    ...
    for it_num in iterations:
        ...
        model_tag = 'model_{}_{}_{}_{}_{}_iter{}'.format(
        track, args.loss, args.num_epochs, args.batch_size, args.lr,it_num)
        ...
        d_label_trn,file_train,path = load_split_dict(it_num)
        print('Iteration --{}--\n\tno. of training trials'.format(it_num),len(file_train))
        
        train_set=Dataset_train(list_IDs = file_train,labels = d_label_trn,base_dir = args.database_path, paths = path)
        ...
        for epoch in range(num_epochs):
            ...
            if train_accuracy > best_acc:
                print('Iteration --{} -- best model found at epoch'.format(it_num), epoch)
