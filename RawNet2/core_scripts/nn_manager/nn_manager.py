...
def f_train_wrapper(args, pt_model, loss_wrapper, device, \
    ...
    for epoch_idx in range(start_epoch, epoch_num):
        ...
        # save the best model
        if flag_new_best:
            tmp_best_name = nii_nn_tools.f_save_trained_name(args, suffix=it_num)
            tm = "/".join(tmp_best_name.split("/")[:-1])
        ...
        if not args.not_save_each_epoch:
            tmp_model_name = nii_nn_tools.f_save_epoch_name(args, epoch_idx, suffix=it_num)
    ...
    nii_display.f_print("{}".format(nii_nn_tools.f_save_trained_name(args, suffix=it_num)))
    return
