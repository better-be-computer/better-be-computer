...
def load_split_dict(number):
    ...
    audio_location = "/data/ASVspoof/ASVspoofEval/pkl"
    process_files = []
...
if __name__ == '__main__':
    ...
    if args.use_pretrained:
        mod_name = args.pretrained_path
        mod_name_base = os.path.splitext(os.path.basename(mod_name))[0]
        model.load_state_dict(torch.load(mod_name,map_location=device))

        # eval_files = load_eval_pkl()[0]

        file_eval = open(args.protocol_path, 'r').readlines()
        file_eval = [item.strip() for item in file_eval]
        # print('no. of eval trials',len(file_eval))
        eval_set=Dataset_gen(list_IDs = file_eval,base_dir = args.database_path)

        # eval_name = os.path.basename(eval_files).replace(".pkl", "")
