...
def load_split_dict(number):
    if number == 99:
        d_meta, file_list, path = pkl.load(open("", 'rb'))
        return d_meta, file_list, path
    audio_location = ""
    process_files = []
...
def load_split_dict_extra_training(number):

    dict_location = "
    process_files = []
    for subdir, _, files in os.walk(dict_location):
        for file in files:
            if ("librispeech_{}".format(number) in file):
                process_files.append(os.path.join(subdir,file))

    process_files = sorted(process_files)

    # print(process_files[number])
    d_meta, file_list, path = pkl.load(open(process_files[0], 'rb'))

    return d_meta, file_list, path, process_files[0]
...
class Dataset_train(Dataset):
    ...
    def __getitem__(self, index):
        ...
        # path = os.path.basename(path).replace(".wav",".flac")
...
if __name__ == '__main__':
    ...
    # parser.add_argument('--iteration_numbers', type=str, default="0")
    ...
    # iterations = args.iteration_numbers
